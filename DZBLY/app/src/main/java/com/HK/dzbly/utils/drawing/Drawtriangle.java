...
public class Drawtriangle extends View {
    ...
    private void initGarden(Canvas canvas,double x,double y) {
       /* p.setColor(0xff0000ff);
        ...
        //参数1,2确定圆心的位置, 参数3.半径
        canvas.drawCircle(200,600,15,p);

        Path path = new Path();
        p.setAntiAlias(true);
        //path.moveTo(40, 650);// 此点为多边形的起点 A
        //path.lineTo(300, 650);   //B
        RectF rectF = new RectF(150, 550, 250, 650);
        path.addArc(rectF,-310,300);
        //canvas.drawPath(path, p);
        p.setTextSize(25);
        canvas.drawTextOnPath("置位在所器仪",path ,30f,0f,p);*/
