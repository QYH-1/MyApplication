...
public class LabelMaker {
    ...
    //绘制字符串
    private static final int STATE_ARAWING = 3;
    ...
    public void beginAdding(GL10 gl){
       // chfckState(STATE_INITIALIZED,STATE_ADDING);
        //清理所有标签
        mLabels.clear();
        mU = 0;
        mV = 0;
    ...
    //添加标签（过来，背景图片，字符串，画笔）
    private int add(GL10 gl, Drawable background, String text, Paint textPaint) {
        return add(gl,background,text,textPaint,0,0);
    }
    //添加一个字符串为空白标签
    private int add(GL10 gl, Drawable drawable,int minWidth, int minHeight) {
        return add(gl,drawable,null,null,minWidth,minHeight);
    }
    //添加标签（gl,背景图片，字符串，画笔，最小宽度，最小高度）
    private int add(GL10 gl, Drawable background, String text, Paint textPaint, int minWidth, int minHeight) {
       // chfckState(STATE_ADDING,STATE_ADDING);
        ...
        //绘制字符串
        if(drawText){
            mCanvas.drawText(text,u + padding.left + centerOffsetWidth,vBase + padding.top+centerOffsetHeight,mClearPaint);
        }
        mU = u + width;
        mV = v;
    ...
    public void endAdding(GL10 gl){
       // chfckState(STATE_ADDING,STATE_INITIALIZED);
        //绑定我们绘制的纹理
        gl.glBindTexture(GL10.GL_TEXTURE_2D,mTextureID);
        //装载纹理
    ...
    public void beginDrawing(GL10 gl,float viewWidth,float viewHeight){
       ...
       // Log.d("viewWidth", String.valueOf(viewWidth));
        ...
        //chfckState(STATE_INITIALIZED,STATE_ARAWING);
        gl.glBindTexture(GL10.GL_TEXTURE_2D,mTextureID);
    ...
    public void draw(GL10 gl,float x,float y,int labelID){
        //chfckState(STATE_ARAWING,STATE_ARAWING);
     ...
     //   Log.d("label.mCrop", String.valueOf(label.mCrop));
    ...
    public void endDrawing(GL10 gl){
        //chfckState(STATE_ARAWING,STATE_INITIALIZED);
    ...
    //检查状态是否正确
//    private void chfckState(int oldState,int newState){
//        if(mState !=oldState){
